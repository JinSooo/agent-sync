...
- stale trusted-recipient warnings and rotation playbooks in desktop and CLI. The plan tells you when to generate a new key on the remote device, save the new public recipient, verify a new encrypted bundle, and then forget the old local profile; it does not remotely revoke old private keys.
