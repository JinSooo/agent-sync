...
Still to deepen:
- Codex native session import/remap into Codex-owned indexes/databases; current store discovery can inventory candidates and SQLite schema metadata, but direct database/index writes remain adapter-specific future work and are not advertised by current capabilities.
- Claude Code native session import/remap into Claude-owned indexes/databases; current store discovery can inventory candidates and SQLite schema metadata, but direct database/index writes remain adapter-specific future work and are not advertised by current capabilities.
